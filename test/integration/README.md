...
`s3Upload.test.js` is the same shape again, for the S3 asset-storage work (`STORAGE_PROVIDER=s3`, `src/infrastructure/storage/`). It needs no AWS credentials either — it runs against an S3-compatible server, and skips itself cleanly if nothing answers at `S3_TEST_ENDPOINT` (default `http://localhost:9000`). Locally that's MinIO via `docker compose up -d` (`docker-compose.yml`); in CI it's a plain `docker run` step in `.github/workflows/ci.yml` rather than a `services:` entry, because that schema has no field for the `server /data` command the MinIO image requires. Both pull from **quay.io**, not Docker Hub: MinIO has removed `minio/minio` and `minio/mc` from Docker Hub (an anonymous pull now fails with "pull access denied … repository does not exist or may require `docker login`"), and publishes to `quay.io/minio/*` instead. Verified manually against a real MinIO container and a real `bin/build-index.js` run as of 2026-08-27.
